...
const MapBox = ({ listAddress, show, isOneLocation, data }: Props): JSX.Element => {
  ...
  useEffect(() => {
    if (selectedLocationRef.current && !isOneLocation) {
      if (listAddress) {
  ...
  const handleDeleteAddressListLocation = (locationId: string) => {
    ...
    if (selectedLocation) {
      ...
      arrayCoordinates.map((item) => {
        map.on('load', () => {
          ...
          if (isOneLocation || arrayCoordinates.length === 1) {
  ...
  const checkDisabledButton = () => {
    if (listAddress !== undefined)
      return isOneLocation ? listAddress.length + 1 > 5 : listAddress.length + listLocation.length > 5
